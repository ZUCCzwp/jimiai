	...
	for i, z := range data {
		...
		if user, ok := userMap[userId]; ok {
			result = append(result, &inviteModel.UserRank{
				...
				Score:    z.Score,
				Coins:    int64(user.Payment.JiyuCoin),
...
func UpdateUserScore(userId int64, coins int) error {
	...
	user.Payment.JiyuCoin = coins
	if err = userRepo.UpdateJiyuCoinTx(tx, user); err != nil {
		tx.Rollback()
	...
	// Redis更新
	sumCoins := coins + user.Payment.JiyuCoin
