...
func GetStructJsonTags[T any](s T) []string {
...
func IsImageExt(ext string) bool {
	exts := []string{".jpg", ".jpeg", ".png", ".JPG", ".JPEG", ".PNG"}

	return lo.Contains(exts, ext)
}
...
func GenerateRandomNickname() string {
	...
	// 随机生成一个长度为2~3的字符串作为昵称补充
	extraPart := generateRandomString(rand.Intn(2) + 2)
	nickname := surname + name + extraPart
	return nickname
}
...
// GenerateRandomString 生成指定长度的随机字符串（公开函数）
func GenerateRandomString(length int) string {
	rand.Seed(time.Now().UnixNano())
...
func GenerateRandomAvatar() string {
	...
	// 格式化为三位数，包含前导零
	randomString := fmt.Sprintf("%03d", randomNumber)

	return qiniu.CdnDomain + "avatar/" + randomString + ".png"
}

// FormatTimeDifference 计算时间
func FormatTimeDifference(duration time.Duration) string {
	if duration < time.Minute {
		...
		return time.Now().Add(-duration).Format("01-02")
	} else {
		return time.Now().Add(-duration).Format("2006-01-02")
	}
}

// 订单ID：年月日时分秒+毫秒时间戳后3位+3位随机数字
func GenerateOrderId() uint64 {
	now := time.Now()
	return uint64(cast.ToInt64(now.Format("060102150405"))*1000000 + now.UnixMicro()%1000 + (int64(uuid.New().ID()) % 1000))
}
